...
def create_validation_figure(
    output_path: Path,
    spde_specs: Sequence[tuple[type[LinearSPDE], dict, str]] | None = None,
    config: SimulationConfig = SimulationConfig(),
    seed: int = RANDOM_SEED,
) -> plt.Figure:
    """
    Create publication-quality figure validating the score formula.
    
    Generates a multi-panel figure showing score evolution along sample paths
    for multiple SPDE types, with error subplots demonstrating agreement
    between Malliavin formula and finite differences.
    
    Args:
        output_path: Path for saving the figure (without extension)
        spde_specs: List of (class, kwargs, title) tuples. If None, uses defaults.
        config: Simulation configuration
    ...
    # Default: all 8 SPDE types
    if spde_specs is None:
        spde_specs = [
            # Second-order
            ...
            # General
            (PolynomialLaplacian, {"poly": PolynomialCoefficients({2: -1.0, 1: 0.5})}, 
             "Polynomial"),
        ]
    
    n_spdes = len(spde_specs)
    n_cols = 4
    n_rows = 2
    ...
    # Create outer grid: 2 rows of SPDE groups
    ...
    # First pass: compute all data and find global error range
    all_results = []
    all_errors = []
    
    print(f"{'SPDE':<30} {'Error Min':>12} {'Error Max':>12} {'Median':>12}")
    print("-" * 70)
    
    for idx, (spde_class, spde_kwargs, title) in enumerate(spde_specs):
        ...
        print(f"{title:<30} {errors.min():>12.2e} {errors.max():>12.2e} {np.median(errors):>12.2e}")
    ...
    print("-" * 70)
    print(f"Error y-axis limits: [{y_min:.2e}, {y_max:.2e}]")
    ...
    for idx, (times, mall_scores, fd_scores, errors, title) in enumerate(all_results):
        ...
        # Create inner grid for each SPDE (main plot + error plot)
        ...
        ax_main = fig.add_subplot(inner_grid[0, col])
        ax_error = fig.add_subplot(inner_grid[1, col])
        ...
        for path_idx in range(config.n_paths):
            ...
            ax_main.plot(
                ...
                "-", color=colour, linewidth=2.0, alpha=0.9
            )
        ...
        ax_main.set_title(title, fontweight="bold", fontsize=12, pad=8)
        ax_main.set_xlim([0, config.t_end + 0.02])
        ax_main.tick_params(labelbottom=False)
        ax_main.set_ylabel(r"Score $\beta_h(u)$", fontsize=10)
        ...
        ax_error.set_xlabel("Time $t$", fontsize=10)
        ...
        if idx == 0:
            legend_elements = [
                Line2D([0], [0], color=COLOURS["grey"], linewidth=2.0, label="Malliavin"),
                Line2D([0], [0], marker="o", color=COLOURS["grey"], markerfacecolor="white",
                       markersize=7, markeredgewidth=1.8, linestyle="None", label="Finite Diff."),
            ]
            ax_main.legend(
        ...
        # Add "Machine ε" label on last panel of first row
        if idx == 3:
            ax_error.text(
                0.97, 2e-12, "Machine ε", 
                fontsize=9, color=COLOURS["grey"], 
                ha="right", style="italic",
                transform=ax_error.get_yaxis_transform(),
            )
    
    # Add row labels
    fig.text(
        0.02, 0.72, "Second-Order SPDEs", 
        fontsize=13, fontweight="bold", 
        rotation=90, va="center", ha="center",
        color=COLOURS["grey"],
    )
    fig.text(
        0.02, 0.28, "Fourth-Order SPDEs", 
        fontsize=13, fontweight="bold", 
        rotation=90, va="center", ha="center",
        color=COLOURS["grey"],
    )
    ...
    # Main title
    fig.suptitle(
        "Malliavin Score Formula Validation Across SPDE Classes",
        fontsize=16,
        fontweight="bold",
        y=0.97,
    )
...
def main() -> None:
    ...
    create_validation_figure(OUTPUT_DIR / "score_trajectories")
